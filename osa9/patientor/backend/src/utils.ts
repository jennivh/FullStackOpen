import { EntryWithoutId, Gender, HealthCheckRating, NewPatient, NewBaseEntry, Diagnosis } from "./types";
...
export const toNewEntry = (object: unknown) : EntryWithoutId => {
    ...
    if('date' in object && 'specialist' in object && 'description' in object && 'type' in object){
        ...
        switch(object.type){
            case 'Hospital':
                if('discharge.date' in object && 'discharge.criteria' in object){
                    const newHospitalEntry : EntryWithoutId = {
                        ...newEntry,
                        type: object.type,
                        discharge: {date: parseDate(object["discharge.date"]), 
            ...
            case 'OccupationalHealthcare':
                if('employerName' in object){
                    if('sickLeave.startDate' in object && 'sickLeave.endDate' in object){
                        return {...newEntry,
                            type: object.type,
                            employerName: parseString(object.employerName),
                            sickLeave: {startDate: parseDate(object['sickLeave.startDate']), 
